...
La implementación inicial usa `abuse_events` como contador D1 de corta retención y guarda únicamente un HMAC de la IP por operación y ventana: 20 intentos/minuto de ingreso, 12 consultas/minuto de ficha TMDB y 30 envíos/minuto de voto público. Es una señal de protección, no una identidad, y no sustituye las restricciones `UNIQUE` ni las cuotas persistentes del negocio. En staging se puede añadir el binding de rate limiting de Workers para absorber ráfagas antes de llegar a D1; sus contadores son aproximados y locales a un centro de datos y sus ventanas deben respetar las admitidas por la API. No construir límites globales estrictos con variables en memoria del Worker. [Rate limiting de Workers](https://developers.cloudflare.com/workers/runtime-apis/bindings/rate-limit/).
