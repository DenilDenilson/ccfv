...
import { closeRound, currentAdmin } from '@/lib/auth/admin';
...
export const POST: APIRoute = async ({ request }) => {
  ...
  return await closeRound(roundId, admin.id) ? redirectTo('/admin?status=round-closed') : errorJson(409, 'conflict', 'La ronda no está abierta.');
};
